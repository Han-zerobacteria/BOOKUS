...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def my_meeting_list(request):
    # 현재 로그인한 사용자가 참여 중인 Membership 조회
    memberships = Membership.objects.filter(user=request.user).select_related('meeting')
    
    # Membership에서 meeting만 추출
    meetings = [membership.meeting for membership in memberships if membership.meeting.delete_status == 'UNDELETE']

    # MeetingSerializer를 이용하여 시리얼라이징
