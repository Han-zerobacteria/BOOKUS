...
def summarize_top1_book():
    books = Book.objects.exclude(best_seller_rank="").order_by('best_seller_rank')[:10]  # 조건을 만족할 후보들을 넉넉히 가져옴
    print(f" 총 후보 책 수: {len(books)}")

    for book in books:
        if not book.content or book.summary:
            continue  # 내용이 없거나 이미 요약된 책은 제외
